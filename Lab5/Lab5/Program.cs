using System;
using System.Collections.Generic;
using System.Linq;

public class UserAccountLab5
{
    public string Name;
    public int Rank;
    public float WinRate;
    public string Type;
    public int Skin;

    public UserAccountLab5(string _name, int _rank, float _winRate, string _type, int _skin)
    {
        Name = _name;
        Rank = _rank;
        WinRate = _winRate;
        Type = _type;
        Skin = _skin;
    }

    public static void Main()
    {
        List<UserAccountLab5> user_list = new List<UserAccountLab5>();

        user_list.Add(new UserAccountLab5("Sơn Tùng", 10, 55.5f, "Ca Nhạc", 50));
        user_list.Add(new UserAccountLab5("Đen Vâu", 5, 70f, "Ca Nhạc", 10));
        user_list.Add(new UserAccountLab5("Thuỳ Linh", 15, 45.5f, "Ca Nhạc", 25));
        user_list.Add(new UserAccountLab5("Độ Mixi", 1, 90f, "Live", 2));
        user_list.Add(new UserAccountLab5("Bà Tuyết Diamond", 3, 60.5f, "Ẩm Thực", 10));
        user_list.Add(new UserAccountLab5("Free Fire", 2, 55.5f, "Game", 250));
        user_list.Add(new UserAccountLab5("Liên Quân", 10, 65.5f, "Game", 2000));
        user_list.Add(new UserAccountLab5("FO4", 11, 85.5f, "Game", 500));
        user_list.Add(new UserAccountLab5("CSGO", 9, 45.5f, "Game", 300));
        user_list.Add(new UserAccountLab5("PUBG", 12, 85.5f, "Game", 400));

        // Xuất dữ liệu ra màn hình 
        Console.WriteLine("Dữ liệu UserAccount:");
        foreach (UserAccountLab5 user in user_list)
        {
            Console.WriteLine($"Name: {user.Name}, Rank: {user.Rank}, Win Rate: {user.WinRate}, Type: {user.Type}, Skin: {user.Skin}");
        }

        // Sắp xếp danh sách user sử dụng LinQ OrderBy 
        var sortedByRank = user_list.OrderBy(u => u.Rank);
        Console.WriteLine("Sắp xếp theo Rank: ");
        foreach (UserAccountLab5 user in sortedByRank)
        {
            Console.WriteLine($"Name: {user.Name}, Rank: {user.Rank}, Win Rate: {user.WinRate}, Type: {user.Type}, Skin: {user.Skin}");
        }

        // Sắp xếp danh sách user sử dụng LinQ OrderBy kết hợp với ThenBy 
        var sortedByNameSkin = user_list.OrderBy(u => u.Name).ThenBy(u => u.Skin);
        Console.WriteLine("Sắp xếp theo Name và Skin: ");
        foreach (UserAccountLab5 user in sortedByNameSkin)
        {
            Console.WriteLine($"Name: {user.Name}, Rank: {user.Rank}, Win Rate: {user.WinRate}, Type: {user.Type}, Skin: {user.Skin}");
        }

        // Liệt kê danh sách người chơi có username bắt đầu bằng chữ “B” 
        var usersWithB = user_list.Where(u => u.Name.StartsWith("B"));
        Console.WriteLine("Danh sách người chơi có Name bắt đầu bằng chữ 'B':");
        foreach (UserAccountLab5 user in usersWithB)
        {
            Console.WriteLine($"Name: {user.Name}, Rank: {user.Rank}, Win Rate: {user.WinRate}, Type: {user.Type}, Skin: {user.Skin}");
        }
    }
}